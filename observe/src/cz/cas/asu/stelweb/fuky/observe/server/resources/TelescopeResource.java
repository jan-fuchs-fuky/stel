...
public class TelescopeResource {
    ...
    public TelescopeResource(@Context SecurityContext sc,
        ...
        try {
            ...
            config.setServerURL(new URL(
                String.format("http://%s:%s/RPC2",
                    servletContext.getInitParameter("observe.xmlrpc.telescope.host"),
                    servletContext.getInitParameter("observe.xmlrpc.telescope.port")
                ))
            );
            ...
            servletContext.log("TelescopeResource()", e);
    ...
    public String get() {
        ...
        try {
            untypedResult = this.client.execute("telescope_info", new Object[]{});
        }
        catch (Exception e) {
            servletContext.log("TelescopeResource.get()", e);
        ...
        try {
            TelescopeInfoXML telescopeInfoXML = new TelescopeInfoXML((HashMap)untypedResult);

            return object2xml(telescopeInfoXML, TelescopeInfoXML.class);
        }
        catch (Exception e) {
            servletContext.log("TelescopeResource.get()", e);
            throw new WebApplicationException(Status.INTERNAL_SERVER_ERROR);
    ...
    public String execute(JAXBElement<TelescopeExecuteXML> jaxbTelescopeExecuteXML) {
        if ((!sc.isUserInRole("admin")) && (!sc.isUserInRole("control"))) {
            throw new WebApplicationException(Status.FORBIDDEN);
        }

        TelescopeExecuteXML telescopeExecuteXML = jaxbTelescopeExecuteXML.getValue();

        try {
            String result = (String)this.client.execute(
                telescopeExecuteXML.getFunctionName(), telescopeExecuteXML.getFunctionParams());
            telescopeExecuteXML.setResult(result);
        }
        catch (Exception e) {
            servletContext.log("TelescopeResource.execute()", e);
            throw new WebApplicationException(Status.SERVICE_UNAVAILABLE);
        }

        return object2xml(telescopeExecuteXML, TelescopeExecuteXML.class);
    }
