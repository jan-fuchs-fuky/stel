...
// |  ReMapCommonBuffer
// +----------------------------------------------------------------------------
// |  Remaps the kernel image buffer by first calling UnMapDCommonBuffer if
// |  necessary and then calls MapDevice.
// |
...
// +----------------------------------------------------------------------------
ulong CArcDevice::CommonBufferPA()
{
...
void CArcDevice::SetupController( bool bReset, bool bTdl, bool bPower, int dRows,
								  int dCols, const char* pszTimFile, const char* pszUtilFile,
								  const char* pszPciFile, const bool& bAbort )
{
	int dRetVal = 0;
	...
	if ( pszUtilFile != NULL )
	{
		LoadControllerFile( pszUtilFile );
	}
...
// |  <IN> -> bIsBootFile - Set to 'true' if pszFilename represents a "boot" file.
...
// +----------------------------------------------------------------------------
void CArcDevice::LoadControllerFile( const char *pszFilename, bool bValidate, const bool& bAbort )
{
	int dReply = 0;
	...
	if ( IS_ARC12( dReply ) )
	{
		...
		LoadSmallCamControllerFile( pszFilename, false, bAbort );
	}
	else
	{
		LoadGen23ControllerFile( pszFilename, bValidate, bAbort );
	}
...
// |  <IN> -> bAbort      - 'true' to stop; 'false' otherwise. Default: false
// +----------------------------------------------------------------------------
void CArcDevice::LoadSmallCamControllerFile( const char *pszFilename, bool bValidate, const bool& bAbort )
{
	int dAddr	= 0;
	int dData	= 0;
	int dReply	= 0;
...
void CArcDevice::SetImageSize( int dRows, int dCols )
{
	...
	//
#ifdef WIN32
	if ( ( dRows * dCols ) <= ( 4200 * 4200 ) )	// Added on Jan.29.2009 to prevent windows error where system tries
	{											// to allocate new space, so 10560x10560 doesn't use XL code and crashes.
#endif
...
// +----------------------------------------------------------------------------
// | Get Controller Configuration Parameters
// +----------------------------------------------------------------------------
...
// +----------------------------------------------------------------------------
// | Is Controller Configuration Parameter Supported
// +----------------------------------------------------------------------------
			...
			if ( dPCIFrameCount > dLastPCIFrameCount )
			{
				...
				if ( pConIFace != NULL )
				{
					pConIFace->FrameCallback( dFPBCount,
											  ...
											  dRows,
											  dCols,
											  ( ( unsigned char * )CommonBufferSize() )
...
// +----------------------------------------------------------------------------
const char* CArcDevice::GetNextLoggedCmd()
{
	if ( !m_cApiLog.Empty() )
	{
		return m_cApiLog.GetNext();
	}
	else
	{
		return NULL;
...
// |                         file.
// +----------------------------------------------------------------------------
void CArcDevice::LoadTemperatureCtrlData( const char* pszFilename )
{
...
// |                         file to save.
// +----------------------------------------------------------------------------
void CArcDevice::SaveTemperatureCtrlData( const char* pszFilename )
{
...
{
	double* pgTemperature		= new double[ gTmpCtrl_SDNumberOfReads ];
	...
	else if ( dNumberOfReads > 0 )
	{
		...
		{
			if ( fabs( pgTemperature [ i ] - gMeanTemperature ) < gTmpCtrl_SDDegTolerance )
			{
				gTemperatureSum += pgTemperature[ i ];
				dNumberOfValidReads++;
			}
