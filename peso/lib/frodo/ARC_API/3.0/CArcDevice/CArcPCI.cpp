...
#elif defined( linux ) || defined( __APPLE__ )		// Linux/MAC OS X Systems Only
	#include <sys/types.h>
	#include <sys/mman.h>
	#include <sys/ioctl.h>
	#include <fcntl.h>
...
// | Define driver names
// +----------------------------------------------------------------------------
#define AstroPCIClassName		 com_arc_driver_AstroPCI
#define kAstroPCIClassName		"com_arc_driver_AstroPCI"
...
char** CArcPCI::m_pszDevList = NULL;


//#include <fstream>
//std::ofstream dbgStream1( "CArcPCI_Debug.txt" );
...
// +----------------------------------------------------------------------------
const char* CArcPCI::ToString()
{
	return "PCI [ ARC-63 / 64 ]";
}
...
void CArcPCI::FindDevices()
{
	...
	else
	{
		while ( ( pDirEntry = readdir( pDir ) ) != NULL )
		{
			...
			if ( sDirEntry.find( "astro" ) != std::string::npos )
			{
				ArcDev_t tArcDev;
...
// |  UseDevices
// +----------------------------------------------------------------------------
// |  Sets the device binding to a specific value. Used for backward
// |  compatibility with the older astropciV1.7 driver. i.e. allows
// |  "astropci1" to be used on windows.
// |
// |  Throws std::runtime_error on error
// +----------------------------------------------------------------------------
void CArcPCI::UseDevices( const char** pszDeviceList, int dListCount )
{
	//
	// Verify input list
	//
	if ( pszDeviceList == NULL )
	{
		CArcTools::ThrowException( "CArcPCI",
								   "UseDevices",
								   "NULL device list parameter!" );
	}

	//
	// Dump any existing device listings
	//
	if ( !m_vDevList.empty() )
	{
		m_vDevList.clear();
		...
		if ( m_vDevList.size() > 0 )
		{
			CArcTools::ThrowException(
						"CArcPCI",
						"UseDevices",
						"Failed to free existing device bindings!" );
		}
	}
	...
	{
		if ( pszDeviceList[ i ] != NULL )
		{
			woss << L"\\\\.\\"
				 << CArcTools::ConvertAnsiToWide( pszDeviceList[ i ] );

			ArcDev_t tArcDev;
			tArcDev.sName   = CArcTools::ConvertWideToAnsi( woss.str().c_str() );
...
// |  Throws std::runtime_error on error
// +----------------------------------------------------------------------------
const char** CArcPCI::GetDeviceStringList()
{
    if ( !m_vDevList.empty() )
    {
...
// +----------------------------------------------------------------------------
void CArcPCI::FreeDeviceStringList()
{
	if ( m_pszDevList != NULL )
	{
		delete [] m_pszDevList;
...
// |  Throws std::runtime_error on error
// |
// |  <IN> -> pszFile - The PCI '.lod' file to load.
// +----------------------------------------------------------------------------
void CArcPCI::LoadDeviceFile( const char* pszFile )
{
	int dHctrValue	      	 = 0;
	int dWordTotal	      	 = 0;
	int dWordCount			 = 0;
...
int CArcPCI::Command( int dBoardId, int dCommand, int dArg0, int dArg1, int dArg2, int dArg3 )
{
	...
	if ( !dSuccess )
	{
		string sCmd = CArcTools::CmdToString( dCmdData[ 0 ],
											  dBoardId,
			 								  dCmdData[ 1 ],
											  dCmdData[ 2 ],
											  dCmdData[ 3 ],
...
// +----------------------------------------------------------------------------
void CArcPCI::LoadGen23ControllerFile( const char *pszFilename, bool bValidate, const bool& bAbort )
{
	int  dBoardId		= 0;
	int  dType			= 0;
	int  dAddr			= 0;
