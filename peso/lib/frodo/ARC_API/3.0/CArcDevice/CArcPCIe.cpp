...
// +----------------------------------------------------------------------------
	...
	#define AstroPCIeClassName		 com_arc_driver_AstroPCIe
	#define kAstroPCIeClassName		"com_arc_driver_AstroPCIe"
...
{
	return static_cast<bool>( i.sName.compare( j.sName ) );
}
...
char** CArcPCIe::m_pszDevList = NULL;
...
// |  Throws std::runtime_error on error
// +----------------------------------------------------------------------------
void CArcPCIe::UseDevices( const char** pszDeviceList, int dListCount )
{
	CArcTools::ThrowException( "CArcPCIe",
							   "UseDevices",
							   "Not used by PCIe!" );
...
// |  Throws std::runtime_error on error
// +----------------------------------------------------------------------------
const char** CArcPCIe::GetDeviceStringList()
{
    if ( !m_vDevList.empty() )
    {
...
// +----------------------------------------------------------------------------
void CArcPCIe::FreeDeviceStringList()
{
	if ( m_pszDevList != NULL )
	{
		delete [] m_pszDevList;
...
// |  Not used by PCIe.
// +----------------------------------------------------------------------------
void CArcPCIe::LoadDeviceFile( const char* pszFile )
{
	// Not Used !!!
}
...
int CArcPCIe::Command( int dBoardId, int dCommand, int dArg0, int dArg1, int dArg2, int dArg3 )
{
	...
	// +------------------------------------------------------+
	if ( STATUS_READOUT( ReadBar( DEV_REG_BAR, REG_STATUS ) ) )
	{
		CArcTools::ThrowException( "CArcPCIe",
								   "Command",
...
bool CArcPCIe::IsFiberConnected( int dFiberId )
{
	...
	if ( dFiberId == FIBER_A )
	{
		bConnected = STATUS_FIBER_A_CONNECTED( GetStatus() );
	}

	else if ( dFiberId == FIBER_B )
	{
		bConnected = STATUS_FIBER_B_CONNECTED( GetStatus() );
	}
...
// |  Throws std::runtime_error on error
// |
// |  <IN> -> pszFilename   - The TIM or UTIL lod file to load.
...
// +----------------------------------------------------------------------------
void CArcPCIe::LoadGen23ControllerFile( const char *pszFilename, bool bValidate, const bool& bAbort )
{
	int  dBoardId		= 0;
	int  dType			= 0;
	int  dAddr			= 0;
...
int CArcPCIe::SmallCamDLoad( int dBoardId, vector<int>& vData )
{
	...
	try
	{
		...
		//  Send Header
		// +-------------------------------------------------+
		dHeader = ( ( dBoardId << 8 ) | ( vData.size() + 1 ) );
...
int CArcPCIe::ReadReply( double gTimeOutSecs )
{
	...
	do
    {
		...
		if ( STATUS_HDR_ERROR( dStatus ) )
		{
			dReply = HERR;
			break;
		}

		else if( STATUS_CONTROLLER_RESET( dStatus ) )
		{
			dReply = SYR;
			break;
		}
    ...
    } while ( !STATUS_REPLY_RECVD( dStatus ) );
...
// |  GetFPGARegString
// +----------------------------------------------------------------------------
// |  Returns the string name for the specified FPGA register offset.
// |
// |  <IN> -> dOffset - The byte offset from the start of PCI config space
// +----------------------------------------------------------------------------
//string CArcPCIe::GetFPGARegString( int dOffset )
//{
//	string sReg = "";
//
//	switch ( dOffset )
...
//		break;
//
//		case REG_CMD_COMMAND:
//		{
//			sReg = "REG_CMD_COMMAND";
//		}
//		break;
//
//		case REG_CMD_ARG0:
...
//		break;
//
//		case REG_STOP_EXPOSURE:
//		{
//			sReg = "REG_STOP_EXPOSURE";
//		}
//		break;
//
//		case REG_MEMORY_TEST:
//		{
//			sReg = "REG_MEMORY_TEST";
//		}
//		break;
//
//		case REG_INIT_IMG_ADDR:
//		{
//			sReg = "REG_INIT_IMG_ADDR";
//		}
//		break;
//
//		case REG_STATUS:
...
//		break;
//
//		case REG_REPLY_HEADER:
//		{
//			sReg = "REG_REPLY_HEADER";
//		}
//		break;
//
//		case REG_REPLY_CMD:
...
//		break;
//
//		case REG_PIXEL_COUNT:
//		{
//			sReg = "REG_PIXEL_COUNT";
//		}
//		break;
//
//		case REG_FRAME_COUNT:
//		{
//			sReg = "REG_FRAME_COUNT";
//		}
//		break;
//
//		case REG_ID_LO:
...
//		break;
//
//		case REG_CONTROLLER_CMD:
//		{
//			sReg = "REG_CONTROLLER_CMD";
//		}
//		break;
//
//		default:
