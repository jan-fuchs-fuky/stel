...
std::queue<std::string>::size_type CLog::Q_MAX = 1024;	// Maximum number of messages to log
...
char* CLog::GetNext()
{
	...
	{
#ifdef WIN32
		CopyMemory( m_szBuffer,
					m_sQueue.front().c_str(),
					m_sQueue.front().size() );
#else
		strcpy( m_szBuffer, m_sQueue.front().c_str() );
#endif
...
char* CLog::GetLast()
{
	if ( !m_sQueue.empty() )
	{
		...
		CopyMemory( m_szBuffer,
					...
					m_sQueue.back().size() );
#else
		strcpy( m_szBuffer, m_sQueue.back().c_str() );
#endif
