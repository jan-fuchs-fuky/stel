...
namespace arc
{
	...
	#define PCI_GET_STATUS_GET_DEVSEL_STRING( x )	( ( ( ( x & 0x0600 ) == 0x0000 ) > 0 ? "fast" :     \
													( ( ( ( x & 0x0600 ) == 0x0100 ) > 0 ? "medium" :   \
													( ( ( ( x & 0x0600 ) == 0x0200 ) > 0 ? "slow" :     \
													( ( ( ( x & 0x0600 ) == 0x0500 ) > 0 ? "reserved" : \
													"unknown" ) ) ) ) ) ) ) )
	...
	#define PCI_GET_BASE_ADDR_MEM_TYPE_STRING( x )	( ( ( PCI_GET_BASE_ADDR_TYPE( x ) == 0x0 ) ? "locate anywhere 32-bit addr space" :     \
													( ( ( PCI_GET_BASE_ADDR_TYPE( x ) == 0x1 ) ? "locate below 1 Meg" :                    \
													( ( ( PCI_GET_BASE_ADDR_TYPE( x ) == 0x2 ) ? "locate anywhere 64-bit addr space" :     \
													"reserved" ) ) ) ) ) )
