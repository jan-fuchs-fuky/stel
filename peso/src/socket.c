...
int socket_server_create(char *p_ip, int port, int *p_sockfd)
{
    ...
    if (setsockopt(*p_sockfd, IPPROTO_TCP, TCP_MAXSEG, (char *) &flag_sockopt,
            sizeof(flag_sockopt)) < 0)
    {
        return -1;
    }
    ...
    flag_sockopt = 1;
    if (setsockopt(*p_sockfd, SOL_SOCKET, SO_REUSEADDR, (char *) &flag_sockopt,
            sizeof(flag_sockopt)) < 0)
    {
        return -1;
    }

    if (bind(*p_sockfd, (struct sockaddr *) &server_address,
